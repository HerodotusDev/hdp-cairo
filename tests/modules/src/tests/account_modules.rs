...
async fn test_modules_get_balance() {
    run(serde_json::from_slice(include_bytes!("../../../../target/dev/modules_get_balance.compiled_contract_class.json")).unwrap()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn test_modules_get_code_hash() {
    run(serde_json::from_slice(include_bytes!(
        "../../../../target/dev/modules_get_code_hash.compiled_contract_class.json"
    ))
    .unwrap())
    .await
}
