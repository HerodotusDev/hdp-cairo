...
func test_receipt_decoding_inner{
    ...
    let success = ReceiptDecoder.get_field(rlp, ReceiptField.SUCCESS, rlp_start_offset, tx_type, block_number);
    assert success.low = expected_success.low;
    assert success.high = expected_success.high;

    let cumulative_gas_used = ReceiptDecoder.get_field(rlp, ReceiptField.CUMULATIVE_GAS_USED, rlp_start_offset, tx_type, block_number);
    ...
    }(expected_bloom, expected_bloom_len, expected_bloom_bytes_len, rlp, ReceiptField.BLOOM, rlp_start_offset, tx_type, block_number);

    eval_felt_field{
        range_check_ptr=range_check_ptr, bitwise_ptr=bitwise_ptr, pow2_array=pow2_array
    }(expected_logs, expected_logs_len, expected_logs_bytes_len, rlp, ReceiptField.LOGS, rlp_start_offset, tx_type, block_number);
...
}(expected: felt*, expected_len: felt, expected_bytes_len: felt, rlp: felt*, field: felt, rlp_start_offset: felt, tx_type: felt, block_number: felt) {
    alloc_locals;

    let (res, res_len, res_bytes_len) = ReceiptDecoder.get_felt_field(rlp, field, rlp_start_offset, tx_type);
