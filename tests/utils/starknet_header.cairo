...
func test_starknet_header_decoding{range_check_ptr}(header_len: felt, index: felt) {
    ...
    %{
        ...
        load_dotenv()
        RPC_URL_STARKNET = os.getenv("STARKNET_RPC_URL", "https://pathfinder.sepolia.iosis.tech/")
        FEEDER_URL = os.getenv("STARKNET_FEEDER_URL", "https://alpha-sepolia.starknet.io/feeder_gateway/")

        if RPC_URL_STARKNET is None:
            raise ValueError("STARKNET_RPC_URL environment variable is not set")
        if FEEDER_URL is None:
            raise ValueError("STARKNET_FEEDER_URL environment variable is not set")

        provider = StarknetProvider(RPC_URL_STARKNET, FEEDER_URL, 1)
