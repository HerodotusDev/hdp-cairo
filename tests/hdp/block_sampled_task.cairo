...
func test_block_sampled_task_param_decoding{
    ...
    // AVG:
    let (expected_avg_task, avg_input, avg_bytes_len, avg_datalake) = BlockSampledTaskMocker.get_avg_task();
    let (avg_task) = extract_params_and_construct_task(avg_input, avg_bytes_len, avg_datalake);
    task_eq(avg_task, expected_avg_task);

    // SUM:
    let (expected_sum_task, sum_input, sum_bytes_len, sum_datalake) = BlockSampledTaskMocker.get_sum_task();
    let (sum_task) = extract_params_and_construct_task(sum_input, sum_bytes_len, sum_datalake);
    task_eq(sum_task, expected_sum_task);

    // MIN:
    let (expected_min_task, min_input, min_bytes_len, min_datalake) = BlockSampledTaskMocker.get_min_task();
    let (min_task) = extract_params_and_construct_task(min_input, min_bytes_len, min_datalake);
    task_eq(min_task, expected_min_task);

    // MAX:
    let (expected_max_task, max_input, max_bytes_len, max_datalake) = BlockSampledTaskMocker.get_max_task();
    let (max_task) = extract_params_and_construct_task(max_input, max_bytes_len, max_datalake);
    task_eq(max_task, expected_max_task);

    // COUNT_IF:
    let (expected_count_if_task, count_if_input, count_if_bytes_len, count_if_datalake) = BlockSampledTaskMocker.get_count_if_task();
    let (count_if_task) = extract_params_and_construct_task(count_if_input, count_if_bytes_len, count_if_datalake);
