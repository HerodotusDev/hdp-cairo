...
namespace BlockSampledTask {
    func init{
        ...
        if (index == n_tasks) {
            ...
            assert block_sampled_tasks[index] = BlockSampledComputationalTask(
                aggregate_fn_id=aggregate_fn_id, hash=hash, datalake=datalake
            );
    ...
    func execute{
        ...
        if (tasks[index].aggregate_fn_id == 0) {
            let result = compute_avg{range_check_ptr=range_check_ptr}(
        ...
        if (tasks[index].aggregate_fn_id == 1) {
            let result = compute_sum{range_check_ptr=range_check_ptr}(
                values=data_points, values_len=data_points_len
            );
...
// Internal Functions:
func extract_params{range_check_ptr}(input: felt*) -> (
    datalake_hash_low: felt, datalake_hash_high: felt, aggregate_fn_id: felt
) {
    alloc_locals;
    ...
    if (task == 0x677661) {
        return (
            datalake_hash_low=datalake_hash_low,
            datalake_hash_high=datalake_hash_high,
            aggregate_fn_id=0,
        );
    }
