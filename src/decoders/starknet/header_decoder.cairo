...
func get_header_field_index{range_check_ptr}(version: felt, field: felt) -> felt {
    ...
    dw 8;  // TRANSACTION_COMMITMENT (not available in v2)
    dw 6;  // EVENT_COUNT (part of the same field as TRANSACTION_COUNT in v2)
    dw 9;  // EVENT_COMMITMENT (not available in v2)
    dw 16;  // PARENT_BLOCK_HASH
    dw 7;  // STATE_DIFF_COMMITMENT
    dw 5;  // STATE_DIFF_LENGTH
    dw 10;  // L1_GAS_PRICE_IN_WEI
    dw 11;  // L1_GAS_PRICE_IN_FRI
    dw 12;  // L1_DATA_GAS_PRICE_IN_WEI
    dw 13;  // L1_DATA_GAS_PRICE_IN_FRI
    dw 14;  // RECEIPTS_COMMITMENT
    dw 6;  // L1_DATA_MODE
}
