    ...
    fn execute(&mut self, request: Self::Request, vm: &mut VirtualMachine) -> SyscallResult<Self::Response> {
        ...
        match call_handler_id {
            CallHandlerId::Header => {
                ...
                let result = header::HeaderCallHandler.handle(key.clone(), function_id, vm)?;
            ...
            CallHandlerId::Account => {
                ...
                let result = account::AccountCallHandler.handle(key.clone(), function_id, vm)?;
            ...
            CallHandlerId::Storage => {
                ...
                let result = storage::StorageCallHandler.handle(key.clone(), function_id, vm)?;
