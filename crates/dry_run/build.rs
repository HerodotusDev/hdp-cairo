...
fn main() {
    ...
    let gen_path = PathBuf::from(env::var("OUT_DIR").expect("OUT_DIR is not set")).join("compiled_dry_run_program.rs");
    fs::write(
        &gen_path,
        format!(
            "pub const COMPILED_DRY_RUN_PROGRAM: &[u8] = include_bytes!(r#\"{}\"#);",
            output_file.display()
        ),
    )
