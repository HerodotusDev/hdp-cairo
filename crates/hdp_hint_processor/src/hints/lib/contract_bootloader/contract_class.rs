...
pub fn write_class(vm: &mut VirtualMachine, ptr: Relocatable, contract_class: CasmContractClass) -> Result<(), HintError> {
    vm.insert_value(ptr, COMPILED_CLASS_V1)?;

    load_casm_entrypoints(vm, (ptr + 1)?, &contract_class.entry_points_by_type.external)?;
    load_casm_entrypoints(vm, (ptr + 3)?, &contract_class.entry_points_by_type.l1_handler)?;
    load_casm_entrypoints(vm, (ptr + 5)?, &contract_class.entry_points_by_type.constructor)?;

    let bytecode: Vec<_> = contract_class.bytecode.iter().map(|x| x.value.clone()).collect();

    let data: Vec<MaybeRelocatable> = bytecode.into_iter().map(Felt252::from).map(MaybeRelocatable::from).collect();
...
fn load_casm_entrypoints(vm: &mut VirtualMachine, base: Relocatable, entry_points: &[CasmContractEntryPoint]) -> Result<(), HintError> {
