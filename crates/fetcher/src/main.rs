...
use fetcher::{infer_mmr_sources_from_indexer, parse_syscall_handler, Args, Fetcher};
use futures as _;
...
async fn main() -> Result<(), fetcher::FetcherError> {
    ...
    let mmr_hasher_config = if let Some(path) = args.mmr_hasher_config.as_ref() {
        serde_json::from_slice(&fs::read(path)?)?
    } else {
        infer_mmr_sources_from_indexer(&proof_keys).await?
    };

    let fetcher = Fetcher::new_with_mmr_sources_map(&proof_keys, mmr_hasher_config);
    let (
        eth_proofs_mainnet,
        eth_proofs_sepolia,
