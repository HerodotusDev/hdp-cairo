...
contract SharpFactsAggregatorTest is Test {
    ...
    function setUp() public {
        ...
        SharpFactsAggregator.AggregatorState memory initialAggregatorState = SharpFactsAggregator.AggregatorState({
        ...
        assertFalse(sharpFactsAggregator.hasRole(keccak256("OPERATOR_ROLE"), address(this)));
        assertFalse(sharpFactsAggregator.hasRole(keccak256("UNLOCKER_ROLE"), address(this)));
        ...
        assertTrue(sharpFactsAggregator.hasRole(keccak256("OPERATOR_ROLE"), address(this)));
        assertTrue(sharpFactsAggregator.hasRole(keccak256("UNLOCKER_ROLE"), address(this)));
    }
    ...
    function ensureGlobalStateCorrectness(SharpFactsAggregator.JobOutputPacked memory output) internal view {
    ...
    function testRealAggregateJobsFFI() public {
        ...
        sharpFactsAggregator.registerNewRange(pastBlockStart - firstRangeStartChildBlock - 1);

        sharpFactsAggregator.registerNewRange(pastBlockStart - secondRangeStartChildBlock - 1);
        ...
        SharpFactsAggregator.JobOutputPacked[] memory outputs =
            abi.decode(output, (SharpFactsAggregator.JobOutputPacked[]));
        ...
        SharpFactsAggregator.JobOutputPacked[] memory outputsExtended =
            abi.decode(outputExtended, (SharpFactsAggregator.JobOutputPacked[]));

        sharpFactsAggregator.aggregateSharpJobs(secondRangeStartChildBlock + 1, outputsExtended);
        ensureGlobalStateCorrectness(outputsExtended[outputsExtended.length - 1]);
    }
}
