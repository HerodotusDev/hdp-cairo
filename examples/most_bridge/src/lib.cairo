...
mod module {
    ...
    use super::multi_mm_withdrawals_inefficient_dict::{
        BalanceToWithdraw, MultiMMWithdrawalDictTrait, OrdersWithdrawals,
    };
    ...
    pub fn main(
        ...
        // Private input
        mut orders_details: Array<
            OrderFulfillmentDetails,
        > // These orders details array are provided only offchain
        ...
        for order in orders_details {
            ...
            // let mm_withdrawal_address_from_slot = u256_from_words(storage_slot_value_low,
            // storage_slot_value_high)
            // If slot is 0 it means that order is not fulfilled or market maker dont set any
            // withdrawal address
            if (mm_withdrawal_address_from_slot == 0) {
                println!("Order fulfilment verification failure for order {:x}", order_hash);
                tmp_is_verified_correctly = false;
            } else {
                // Sanity check only
                //assert(mm_withdrawal_address_from_slot == order.market_maker_withdrawal_address,
                //'Withdrawal address different');

                if (mm_withdrawal_address_from_slot != order.market_maker_withdrawal_address) {
                    println!(
                        "Withdrawal address different {:x}", order.market_maker_withdrawal_address,
                    );
                }
                ...
                withdrawals_dict
                    .add(mm_withdrawal_address_from_slot, order.source_token, order.source_amount);
            }
            i += 1;
        }
    ...
    fn calculate_order_hash(order_details: OrderFulfillmentDetails) -> u256 {
        let mut bytes: ByteArray = ByteArrayTraitExt::new(0, array![]);
