...
fn bloom_filter_indices(key: u256) -> (usize, usize, usize) {
    let hash: bytes31 = keccak_u256s_be_inputs(array![key].span()).low.into();
    (
...
pub fn contains(bloom: ByteArray, input: u256) -> bool {
    let (index1, index2, index3) = bloom_filter_indices(input);
    let pow_of_2: Array<u8> = array![0x1, 0x2, 0x4, 0x8, 0x10, 0x20, 0x40, 0x80];

    if (bloom[255 - index1 / 8] & *pow_of_2[index1 % 8] == 0) {
        return false;
    }

    if (bloom[255 - index2 / 8] & *pow_of_2[index2 % 8] == 0) {
        return false;
    }

    if (bloom[255 - index3 / 8] & *pow_of_2[index3 % 8] == 0) {
        return false;
    }
